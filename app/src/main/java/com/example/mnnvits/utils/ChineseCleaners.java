...
import com.example.mnnvits.BuildConfig;
...
public class ChineseCleaners {
    private static final String TAG = "ChineseCleaners";
    boolean debug_flag = BuildConfig.DEBUG;
    ...
    public String pinyin_to_prepin(String s) {
        ...
        for (ArrayList<String> j : prepin_to_bopomofo) {
            ...
            Log.d(TAG, bopomofo + " " + j.get(0) + " " + j.get(1));
        }
        return bopomofo;
    }
    ...
    public String chinese_clean_text1(String s) {
        ...
        Log.d(TAG, "pinyin: " + pinyin + " time: " + (end - start) + "ms");
...
//        romaji = romaji.replace(romaji, romaji + " ");
        ...
        String pattern = "[A-Za-zɯɹəɥ→↓↑]";

        if (Pattern.matches(pattern, String.valueOf(lastChar))) {
            romaji += ".";
        }
        sb.append(romaji);
